...
## 2026-05-27 - Optimized Connection Scaling and Python Data Parsing
**Learning:** Scaling the global 'maxIdleConns' to the sum of all backend 'maxIdleConnsPerHost' quotas (e.g., 128000 for 2 backends with 64000 each) prevents global pool contention and ensures each service can fully utilize its connection reuse potential. Furthermore, a Python micro-benchmark verified that manual loops with `item.partition('=')` are approximately 15% faster than nested dictionary comprehensions using `item.split('=', 1)` when parsing Docker configuration strings.
**Action:** Always scale the global connection pool to the aggregate of per-host limits and prefer manual `partition()` loops for performance-sensitive string parsing in Python helpers.
